					...
					<div class="product-views-option-facets-color-picker-small">
						<label class="product-views-option-facets-color-picker-label">
								<a class="product-views-option-facets-color-picker-anchor" >
									{{#if isColorTile}}
										<span data-label="label-{{../cartOptionId}}" data-action="changethumbnail" value="{{this.label}}" class="product-views-option-facets-color-picker-box {{#if isLightColor}}product-views-option-facets-color-picker-box-white-border{{/if}}"
										style="cursor:pointer;background: {{color}}"></span>
									{{else}}
