...
/// <amd-module name="DownloadItems.Model"/>
...
export type DownloadItemsModel = any;

export type DownloadItemsLine = any;

// @class Quote.Model @extends Transaction.Model
export const DownloadItemsModel: any = TransactionModel.extend({
    // @property {String} urlRoot
    urlRoot: 'services/DownloadItems.Service.ss?',
    ...
    submit: function() {
        ...
        const actionId = loggers.start('Request DownloadItems');
        ...
        return this._submit
            ...
            .then(DownloadItems => {
                ...
                const DownloadItemsOperationIds = this.getLatestOperationIds(lastOperationIdIndex);
                let DownloadItemsId = 0;
                if (DownloadItems && DownloadItems.confirmation && DownloadItems.confirmation.internalid) {
                    DownloadItemsId = parseInt(DownloadItems.confirmation.internalid, 10);
                }
                loggers.end(actionId, {
                    operationIds: DownloadItemsOperationIds,
                    status: 'success',
                    transactionId: DownloadItemsId
                });
