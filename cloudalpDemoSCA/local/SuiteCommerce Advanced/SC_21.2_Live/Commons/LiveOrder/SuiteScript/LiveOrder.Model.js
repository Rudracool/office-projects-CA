...
define('LiveOrder.Model', [
    ...
    return SCModel.extend({
        ...
        get: function get(threedsecure) {
            ...
            if (result.summary) {
                  /**
                 * customization on live order model
                 */
                   var subtotalValue;
                   var customData = result.summary.total;
                   if(result.options.custbody_custom_gift_certificate_mess !== '' && result.options.custbody_custom_gift_certificate_mess !== undefined){
                    var shippingCostData = result.summary.total;
                    var shimMethodData = result.summary.shippingcost;
                    var extraShippingCost = (10*shimMethodData)/100;
                    var TotalValue = shippingCostData + extraShippingCost;
                    subtotalValue = TotalValue;
                   } else {
                    subtotalValue = customData;
                   }
                //    if (result.options.custbody_custom_add_shippingcost !== '' && result.options.custbody_custom_add_shippingcost !== undefined) {
                //        var shippingCostData = result.summary.total;
                //        var extraShippingCost = Number(result.options.custbody_custom_add_shippingcost);
                //        var TotalValue = shippingCostData + extraShippingCost;
                //        subtotalValue = TotalValue;
                //    }
                //    else {
                //        subtotalValue = customData;
                //    }
                   result.summary.total = subtotalValue;
